...
class ZKDevice(Document):
	...
	def get_device_logs_direct (self, show_progress=False):
		...
		self.excecution_time = datetime.now().date()
	...
	def get_device_logs_biotime (self, show_progress=False , fetch_next = False):
		try:
			...
			# print(f"\n\n\n last_log_row 1 : {self.last_log_row}")

			if self.last_log_row:
				self.last_log_row = parser.parse(str(self.last_log_row))
    
			# print(f"\n last_log_row 2 : {self.last_log_row}")
			...
			if last:
				self.last_log_row = min(last, datetime.now())
			# print(f"\n last_log_row 3 : {self.last_log_row} \n\n\n")
...
def get_active_device_logs(names=None):
	...
	and ('{cur_time}' >= excecution_time or ifnull(excecution_time,0)=0) ;
	"""
	try:
...
@frappe.whitelist()
def cron_get_active_device_logs(names=None):
    # Call the original function
    get_active_device_logs(names)
